...
model_file_list = [
    (
        "test/records/json_model_test/json_test.json",
        "pydtk.models.json_model.GenericJsonModel",
    ),
    ("test/records/csv_model_test/data/test.csv", "pydtk.models.csv.GenericCsvModel"),
    (
        "test/records/movie_model_test/sample.mp4",
        "pydtk.models.movie.GenericMovieModel",
    ),
]
...
def test_model_generate_metadata(file_path, model):
    ...
    with redirect_stdout(f):
        cli.generate(target="metadata", from_file=file_path, content="abc")
    ...
    f = io.StringIO()
    with redirect_stdout(f):
        cli.generate(target="metadata", model=model, from_file=file_path, content="abc")
    metadata = json.loads(f.getvalue())
    _assert_content(metadata, "abc")
...
def test_list_record_id_with_only_numbers(record_id, database_id):
    ...
    metadata = json.dumps({"record_id": record_id, "path": "/abc", "contents": {}})
