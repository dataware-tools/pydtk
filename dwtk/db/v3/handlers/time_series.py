...
from dwtk.utils.utils import load_config
...
class TimeSeriesDBHandler(_BaseDBHandler):
    ...
    def _initialize_engine(self,
        ...
        engine = db_engine if db_engine is not None \
            else os.environ.get('NUDT_TIME_SERIES_DB_ENGINE', None)
        username = db_username if db_username is not None \
            else os.environ.get('NUDT_TIME_SERIES_DB_USERNAME', None)
        password = db_password if db_password is not None \
            else os.environ.get('NUDT_TIME_SERIES_DB_PASSWORD', None)
        host = db_host if db_host is not None \
            else os.environ.get('NUDT_TIME_SERIES_DB_HOST', None)
        database = db_name if db_name is not None \
            else os.environ.get('NUDT_TIME_SERIES_DB_DATABASE', None)
...
class TimeSeriesCassandraDBHandler(TimeSeriesDBHandler):
    ...
    def _initialize_engine(self,
        ...
        """Initialize DB engine."""
        # Load settings from environment variables
        engine = db_engine if db_engine is not None else os.environ.get('NUDT_TIME_SERIES_DB_ENGINE', None)
        username = db_username if db_username is not None else os.environ.get('NUDT_TIME_SERIES_DB_USERNAME', None)
        password = db_password if db_password is not None else os.environ.get('NUDT_TIME_SERIES_DB_PASSWORD', None)
        host = db_host if db_host is not None else os.environ.get('NUDT_TIME_SERIES_DB_HOST', None)
        database = db_name if db_name is not None else os.environ.get('NUDT_TIME_SERIES_DB_DATABASE', None)
        ...
        cluster = Cluster(contact_points=[hostname],
                          port=int(hostport),
                          connect_timeout=int(os.environ.get('NUDT_CASSANDRA_CONNECT_TIMEOUT', 60)),
