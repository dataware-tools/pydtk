...
from dwtk.db import V2BaseDBHandler as _V2BaseDBHandler
from dwtk.utils.utils import load_config
from dwtk.utils.utils import dicts_to_listed_dict_2d
from dwtk.utils.utils import dtype_string_to_dtype_object
from dwtk.utils.utils import serialize_dict_1d
from dwtk.utils.utils import deserialize_dict_1d
    ...
    for filename in os.listdir(os.path.join(os.path.dirname(__file__))):
        ...
        try:
            importlib.import_module(
                os.path.join('dwtk.db.v3.handlers',
...
class BaseDBHandler(_V2BaseDBHandler):
    ...
    def _get_handler(cls, db_class, db_engine=None):
        ...
        if db_engine is None:
            db_engine = os.environ.get('NUDT_{}_DB_ENGINE'.format(db_class.upper()), None)
    ...
    def _append_listed_dict_to_df(self, data, check_unique=False):
        ...
        """
        self.df = pd.concat(
            [self.df, pd.DataFrame.from_dict(data).set_index('uuid_in_df')],
            sort=False
        )
        if check_unique:
    ...
    def _remove_duplicates(self):
        ...
        q = 'create table {0} as ' \
            ...
            ') ' \
            'group by uuid_in_df'. \
            format(self._quote(temp_table_name),
        ...
        if self._config.current_db['engine'] in ['sqlite']:
            ...
            raise ValueError('Unsupported engine: {}'.format(
                self._config.current_db['engine']
            ))
        self._engine.execute(q)
        ...
        self._engine.execute('alter table {0} rename to {1}'.
                             format(self._quote(self.df_name),
                                    self._quote(self.df_name + '_deprecated')))

        # Rename the temporal table
        self._engine.execute('alter table {0} rename to {1}'.
                             format(self._quote(temp_table_name), self._quote(self.df_name)))
