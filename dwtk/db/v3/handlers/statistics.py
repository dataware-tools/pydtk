...
from .time_series import TimeSeriesDBHandler
from .time_series import TimeSeriesCassandraDBHandler
...
class StatisticsDBHandler(TimeSeriesDBHandler):
    ...
    def _initialize_engine(self,
        ...
        engine = db_engine if db_engine is not None \
            else os.environ.get('NUDT_STATISTICS_DB_ENGINE', None)
        username = db_username if db_username is not None \
            else os.environ.get('NUDT_STATISTICS_DB_USERNAME', None)
        password = db_password if db_password is not None \
            else os.environ.get('NUDT_STATISTICS_DB_PASSWORD', None)
        host = db_host if db_host is not None \
            else os.environ.get('NUDT_STATISTICS_DB_HOST', None)
        database = db_name if db_name is not None \
            else os.environ.get('NUDT_STATISTICS_DB_DATABASE', None)
